...
## Teardown → Setup: what gets recreated

If you have just run [scripts/teardown-environment.ps1](../scripts/teardown-environment.ps1) (or are reading from a clean clone), this is the conceptual map of what setup brings back. It is intentionally not tied to step numbers — those tend to drift across script versions; the artifact mapping does not.

| Removed by teardown | Recreated by setup | Same name? | Notes |
|---|---|---|---|
| Blueprint app + service principal | `a365 setup all --skip-infrastructure --skip-requirements` | Yes (display name `<AgentName> Blueprint`) | New `appId` / `objectId` — Entra never reuses GUIDs. |
| Client app + service principal | `az ad app create` + `az ad app permission add` + `az ad app permission admin-consent` | Yes (`<AgentName>-cli-app`) | New `appId`. A fresh client secret is minted later in setup. |
| Agent user (UPN, mailbox, Teams presence) | `a365 create-instance identity -c a365.config.json` | Yes (`<AgentName>@<verified-tenant-domain>`) | The verified domain is resolved from Graph at the start of setup, so the UPN is deterministic. |
| Instance Identity service principals | `a365 create-instance identity` | Yes (`<AgentName> Identity`) | Created in the same call as the agent user. |
| Resource group | `az group create` | Yes (`rg-a365-<AgentName>`) | Same name, new resource. |
| Azure OpenAI account (purged) | `az cognitiveservices account create` + `az cognitiveservices account deployment create` | Yes (`<AgentName>-openai` + deployment `gpt-4o-mini`) | Teardown's purge step is what makes this name immediately reusable instead of waiting 48 h. |
| `a365.config.json` | Generated from CLI parameters + the resolved tenant context, immediately before `a365 setup all` | Yes | All values are deterministic from a single `-AgentName` input. |
| `a365.generated.config.json` | Written by `a365 setup all` and updated by `a365 create-instance identity` | Yes | This is the file with the new `agentBlueprintId`, `AgenticAppId`, and consent record. |
| `env/.env.playground` (template) | Setup writes `CLIENT_APP_ID=<new>` and later flips `ENABLE_A365_OBSERVABILITY_EXPORTER=true` | Yes | Same file, repopulated. |
| `env/.env.playground.user` (template) | Setup writes the Azure OpenAI key + endpoint + deployment, the new `SECRET_CLIENT_APP_SECRET`, the minted `SECRET_OBS_S2S_TOKEN`, and (optionally, interactive) the `SECRET_BEARER_TOKEN` | Yes | Four sub-stages contribute to this file across the run. |

The setup script does **not** write the runtime `.env` at the workspace root — that file is generated by Microsoft 365 Agents Toolkit at F5 / deploy time from the two `env/.env.playground*` files (see [m365agents.playground.yml](../m365agents.playground.yml) → `file/createOrUpdateEnvironmentFile`).

### Why the same names come back

Everything except GUIDs derives from a single input — the `-AgentName` parameter (default `procodeagent`):

```
-AgentName            = "procodeagent"
ResourceGroup         = "rg-a365-<AgentName>"
OpenAI account        = "<AgentName>-openai"
Client app            = "<AgentName>-cli-app"
Blueprint display     = "<AgentName> Blueprint"
Agent identity disp.  = "<AgentName> Identity"
Agent UPN             = "<AgentName>@<verified-tenant-domain>"
```

This naming determinism is what makes a teardown → setup round-trip behave like a fresh install on a different tenant: the only inputs that change are the GUIDs the platform mints, and the secrets / tokens minted alongside them.
